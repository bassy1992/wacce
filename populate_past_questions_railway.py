#!/usr/bin/env python
"""
Script to populate past questions on Railway via API
"""
import requests
import sys

# Railway API credentials
RAILWAY_API_URL = "https://wacce-production.up.railway.app/api"
ADMIN_USERNAME = "railwayadmin"
ADMIN_PASSWORD = "Willfynn1992@"

def populate_past_questions():
    """Populate past questions via Railway API"""
    print("=" * 70)
    print("POPULATING PAST QUESTIONS ON RAILWAY")
    print("=" * 70)
    
    # Login first
    print("\n1. Logging in as admin...")
    login_url = f"{RAILWAY_API_URL}/auth/signin/"
    login_data = {
        "username": ADMIN_USERNAME,
        "password": ADMIN_PASSWORD
    }
    
    session = requests.Session()
    response = session.post(login_url, json=login_data)
    
    if response.status_code == 200:
        print("OK Login successful")
        data = response.json()
        if 'token' in data:
            print("OK Token received")
            # Store token for future requests
            session.headers.update({'Authorization': f"Token {data['token']}"})
    else:
        print(f"ERROR Login failed: {response.status_code}")
        print(response.text)
        return
    
    # Populate past questions
    print("\n2. Populating past questions...")
    populate_url = f"{RAILWAY_API_URL}/past-questions/populate/"
    
    response = session.post(populate_url)
    
    if response.status_code == 200:
        result = response.json()
        print("\nSUCCESS!")
        print("\n" + "=" * 70)
        print("OUTPUT:")
        print("=" * 70)
        print(result.get('output', ''))
        print("\n" + "=" * 70)
        print("Past questions populated successfully on Railway!")
        print("=" * 70)
    else:
        print(f"\nERROR Failed: {response.status_code}")
        print(response.text)

if __name__ == "__main__":
    populate_past_questions()
